...
const Stock = () => {
  ...
  const [stock, setstock] = useState<ClientWithMetrics[]>([]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Visualize e gerencie seu Estoque
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="space-y-4">
          {stock.length === 0 ? (
            ...
            stock.map((stock) => (
              <Card key={stock.id} className="p-6 border-border bg-card hover:border-primary/50 transition-all">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-lg">
                        {stock.nome.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="font-bold text-lg">{stock.nome}</h3>
                        <div className="flex items-center gap-2 text-sm">
                          <button
                            onClick={() => handleWhatsAppClick(stock.whatsapp, stock.nome)}
                            className="text-[#25D366] hover:text-[#20BA5A] hover:underline font-medium transition-colors flex items-center gap-1 group"
                          >
                            <Phone className="h-3 w-3" />
                            <span>{stock.whatsapp}</span>
                    ...
                    <div className="grid md:grid-cols-4 gap-4">
                      <div>
                        ...
                        <p className="font-bold text-primary text-lg">{stock.total_cortes}</p>
                      </div>

                      <div>
                        <p className="text-xs text-muted-foreground">Último Corte</p>
                        <p className="font-semibold">{formatDate(stock.data_ultimo_corte)}</p>
                        {stock.dias_sem_corte !== undefined && (
                          <p className="text-xs text-muted-foreground">
                            {stock.dias_sem_corte} dias atrás
                          </p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{formatDate(stock.data_nascimento)}</p>
                        {stock.is_aniversariante && (
                      ...
                      <div>
                        ...
                        <p className="font-semibold">
                          {stock.proximo_beneficio !== undefined 
                            ? `Faltam ${stock.proximo_beneficio} cortes`
                            : "—"
                          }
                        </p>
                      </div>
